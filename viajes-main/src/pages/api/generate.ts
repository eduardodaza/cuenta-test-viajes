...
async function fetchRapidEvents(city: string, country: string, startDate: string, endDate: string): Promise<any[]> {
  ...
  try {
    ...
    // Elegir el rango más estrecho posible para evitar resultados irrelevantes
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    }).filter((ev: any) => ev.name && (!ev.ts || isNaN(ev.ts) || (ev.ts >= start && ev.ts <= end))).slice(0, 15);
  } catch { return []; }
}

// ── Cache en memoria para EVENTOS (clave: ciudad|start|end) ───
// Evita reconsumir cuota RapidAPI cuando se regenera el mismo viaje.
// TTL 24 h. Se reinicia con cada despliegue.
type EventsCacheEntry = { ts: number; events: unknown[] };
const EVENTS_CACHE: Map<string, EventsCacheEntry> = (global as unknown as { __eventsCache?: Map<string, EventsCacheEntry> }).__eventsCache
  ?? new Map();
...
function cacheKey(city: string, country: string, start: string, end: string) {
  return `${city}|${country}|${start}|${end}`.toLowerCase();
}
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  // EVENTS_MODE: "on" | "cache-only" | "off"
  //   on         → llama RapidAPI siempre que no haya cache vigente
  //   cache-only → solo lee del cache (NUNCA consume cuota RapidAPI). Default en test.
  //   off        → ignora RapidAPI por completo
  const EVENTS_MODE = (process.env.EVENTS_MODE ?? "cache-only").toLowerCase();
  ...
  try {
    ...
    // ── 3. Enriquecimiento — primero Ticketmaster + Wikidata + Weather
    //     (todas son gratis sin cuotas estrictas)
    const [wikidataRes, weatherRes, tmRes] = await Promise.allSettled([
    ...
    // ── 7. RapidAPI — SOLO si hace falta y respetando EVENTS_MODE ──
    ...
    let rapidEvents: unknown[] = [];
    let eventsSource: "cache" | "rapidapi" | "skipped" | "ticketmaster-only" = "ticketmaster-only";

    if (cacheValid) {
      rapidEvents = cached!.events;
      eventsSource = "cache";
    } else if (EVENTS_MODE === "off") {
      eventsSource = "skipped";
    } else if (EVENTS_MODE === "cache-only") {
      // No consumir cuota durante test. Solo Ticketmaster + cache.
      eventsSource = "skipped";
    } else if (EVENTS_MODE === "on") {
      // Solo llamar a RapidAPI si Ticketmaster trajo pocos eventos
      if (tmEvents.length < 5) {
    ...
    pushEvents(tmEvents);
    pushEvents(rapidEvents);
    itinerary.events.sort((a, b) => (a.when ?? "").localeCompare(b.when ?? ""));

    // ── 9. Restaurantes — Google Places (rating real + website oficial)
    ...
    // ── 11. Source tag
    itinerary.generatedBy = `Groq LLaMA 3.3 70B · Ticketmaster · Wikidata · events=${eventsSource}`;
